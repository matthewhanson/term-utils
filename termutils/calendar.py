...
def get_date_labels(items, label_field, date_field='date'):
    # get dictionary of dates and values containing the group names
    dates = sorted(list(set([i[date_field] for i in items])))
    date_labels = {}
    for date in dates:
        date_labels[maya.parse(date).date] = list(set([i[label_field] for i in items if i[date_field] == date]))[0]
    return date_labels


def labeled_calendar(events, cols=3):
